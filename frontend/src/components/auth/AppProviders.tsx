...
function AppProvidersContent({ children }: CommonProvidersProps) {
  ...
  return (
    <>
      <TimezoneInitializer />
      <ComposeProviders providers={CORE_PROVIDERS}>
        ...
        {show404 ? (
          ...
          >
            ...
            <div className="flex h-full w-full gap-1.5 sm:gap-2.5 md:gap-3 overflow-hidden z-10">
              <Sidebar />
              <div className="flex-1 min-w-0 flex flex-col overflow-hidden rounded-lg sm:rounded-xl md:rounded-2xl border border-gray-200/80 dark:border-white/10 bg-[var(--panel)] backdrop-blur-2xl shadow-2xl shadow-indigo-950/20 dark:shadow-black/60 transition-all duration-300 ease-in-out">
                <Header />
                <div
